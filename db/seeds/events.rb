...
# Location types
location_types = ['offline', 'online', 'hybrid']
...
puts "Generating 35 diverse events..."
...
35.times do |i|
  ...
  location = if location_type == 'online'
    ...
    city_data = cities.sample
    {
      ...
      address: "#{rand(100..9999)} #{['Main St', 'Broadway', 'Park Ave', 'First St', 'Market St', 'Tech Blvd', 'Innovation Way'].sample}"
    }
  end
  ...
  when 'Conference'
    event_attrs[:title] = "#{Faker::Company.industry} #{['Annual', 'Global', 'International', 'Leaders', 'Innovation'].sample} Conference #{rand(2023..2026)}"
  when 'Workshop'
    event_attrs[:title] = "#{['Hands-on', 'Practical', 'Interactive', 'Immersive', 'Intensive'].sample} #{Faker::Company.bs.titleize} Workshop"
  when 'Festival'
    event_attrs[:title] = "#{Faker::Address.city} #{['Summer', 'Winter', 'Spring', 'Fall', 'Annual'].sample} #{['Arts', 'Music', 'Food', 'Film', 'Cultural'].sample} Festival"
  when 'Competition'
    event_attrs[:title] = "#{['National', 'International', 'Regional', 'Global', 'Championship'].sample} #{Faker::Game.title} #{['Tournament', 'Competition', 'Challenge', 'Championship'].sample}"
  when 'Seminar'
    event_attrs[:title] = "#{['Advanced', 'Essential', 'Modern', 'Strategic', 'Practical'].sample} #{Faker::Company.bs.titleize} Seminar"
  end
  ...
  puts "✓ Created Event #{i+1}/35: #{event.title}"
end
